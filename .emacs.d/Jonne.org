...
Display Eldoc in a childframe instead of using the modeline.
#+BEGIN_SRC emacs-lisp
  (use-package eldoc-box
    :ensure t
    :config (setopt eldoc-box-offset '(32 32 16)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq
   ;; Reload the contents of directories every time they are visited
   dired-auto-revert-buffer t
   ;; Sort directories first, then apply "natural sorting" to
   ;; prevent dotfiles intermingling with normal files.
